...
with open(cwd + 'normal-word.txt', 'r') as norWord:
    for item in norWord:
        item = item.replace('\n', '')
        if not item in chk:
            words.append([item, np.random.uniform(0, 0.05)])
            chk.append(item)
...
with open(cwd + 'negative-word', 'r') as negWord:
    for item in negWord:
        ...
        item = item.replace('\n', '')
        if not item in chk:
            words.append([item, (-1 * np.random.uniform(0.5, 0.01))])
            chk.append(item)
...
with open(cwd + 'positive-sentiment-words.txt', 'r') as posWord:
    for item in posWord:
        item = item.replace('\n', '')
        if not item in chk:
            words.append([item, (1 * np.random.uniform(0.2, 0.6))])
            chk.append(item)
...
with open(cwd + 'swear-words.txt', 'r') as sweWord:
    for item in sweWord:
        item = item.replace('\n', '')
        if not item in chk:
            words.append([item, (-1 * np.random.uniform(0.7, 1))])
            chk.append(item)
